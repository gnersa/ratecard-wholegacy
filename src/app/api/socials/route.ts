...
export async function PUT(request: Request) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const average = Math.max(0, Number(item.averageViews || item.averageViewsMax || item.averageViewsMin || 0));
    ...
    await sql`insert into social_accounts
      ...
      values (${user.id}, ${platform}, ${handle}, ${url}, ${Number(item.followers || 0)}, ${average}, ${average}, ${average}, ${Number(item.engagementRate || 0)}, ${String(item.contentStyle || "").slice(0,100)}, ${i})`;
  }
