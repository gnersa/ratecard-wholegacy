...
export default function HomeClient() {
  ...
  return (
    <main>
      ...
      <section className="section" id="features"><div className="container"><div className="sectionHeader centeredSectionHeader"><h2>{t.featureTitle}</h2><p>{t.featureLead}</p></div><div className="cards">{t.features.map(([n,title,body])=><article className="card" key={n}><div className="cardNum">{n}</div><h3>{title}</h3><p>{body}</p></article>)}</div></div></section>

      <section className="reviewSection" aria-labelledby="review-title"><div className="container sectionHeader reviewHeader centeredSectionHeader"><h2 id="review-title">{t.reviewsTitle}</h2><p>{t.reviewsLead}</p></div><div className="reviewMarquee"><div className="reviewTrack"><div className="reviewGroup">{reviews.map((r)=><article className="reviewCard" key={`a-${r.name}`}><img src={r.image} alt={r.name}/><div><strong>{r.name}</strong><p>{r[lang]}</p></div></article>)}</div><div className="reviewGroup" aria-hidden="true">{reviews.map((r)=><article className="reviewCard" key={`b-${r.name}`}><img src={r.image} alt=""/><div><strong>{r.name}</strong><p>{r[lang]}</p></div></article>)}</div></div></div></section>
      ...
      <section className="section faqSection" id="faq"><div className="container"><div className="sectionHeader centeredSectionHeader"><h2>{t.faqTitle}</h2><p>{t.faqLead}</p></div><div className="faqGrid">{t.faq.map(([q,a],i)=><details className="faqItem" key={q} open={i===0}><summary>{q}<span aria-hidden="true">+</span></summary><p>{a}</p></details>)}</div></div></section>
